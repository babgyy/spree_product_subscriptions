...
describe Spree::Subscription, type: :model do
  ...
  let(:subscribable_product) { create(:subscribable_product) }
  let(:subscribable_variant) { create(:variant, product: subscribable_product) } 
  ...
  describe "order" do 
    ...
    context "without subscription" do
      it "can complete" do 
        while order_waiting_to_be_completed.next; end
        assert order.complete?
      end
    end

    context "with subscription" do
      let(:pending_order_with_subscription) do
        Spree::Dependencies.cart_add_item_service.constantize.call({
          order: order_waiting_to_be_completed, 
          variant: subscribable_variant, 
          quantity: 1, 
          options: {
            frequency_id: subscribable_product.subscription_frequencies.first.id,
            subscribe: true
          }
        })
        order_waiting_to_be_completed
      end
      
      it "can complete" do 
        while pending_order_with_subscription.next; end
        assert order.complete?
      end
